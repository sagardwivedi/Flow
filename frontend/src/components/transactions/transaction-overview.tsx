...
import type { Transactions } from "@/app/transactions/columns"
...
export function TransactionOverview({ data, isLoading = false }: TransactionOverviewProps) {
  const totalIncome = data.filter((t) => t.amount > 0).reduce((sum, t) => sum + t.amount, 0)

  const totalExpenses = data.filter((t) => t.amount < 0).reduce((sum, t) => sum + t.amount, 0)
        ...
        <CardContent>
          ...
          <div className="flex items-center text-xs text-muted-foreground mt-1">
            <div className={`flex items-center ${netChange >= 0 ? "text-green-500" : "text-red-500"}`}>
              {netChange >= 0 ? <ArrowUpIcon className="mr-1 h-3 w-3" /> : <ArrowDownIcon className="mr-1 h-3 w-3" />}
